  ...
  return (
    <div className="absolute top-0 left-full w-[600px] h-full bg-white border border-slate-200 shadow-xl z-50 p-6 animate-in fade-in slide-in-from-left-2 duration-200">
      <div className="flex justify-between items-center mb-6">
        ...
        <Link to={`/products?category=${category.name}`} className="text-sm text-viet-red font-bold hover:underline flex items-center gap-1">
      ...
      <div className="grid grid-cols-2 gap-x-8 gap-y-6">
        <div>
          ...
          <ul className="space-y-2">
            {category.children.map((child) => (
              <li key={child}>
                <Link to={`/products?subcategory=${child}`} className="text-sm text-slate-600 hover:text-viet-red transition-colors">
                  {child}
                </Link>
              </li>
            ))}
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              {[1, 2].map((i) => (
                <div key={i} className="group cursor-pointer">
                  ...
                  <p className="text-[11px] text-slate-600 line-clamp-1 group-hover:text-viet-red">Trending {category.name} Item</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-slate-50 p-4 border border-slate-100">
            ...
            <Link to="/suppliers" className="text-[10px] font-bold text-viet-red hover:underline">Browse Suppliers →</Link>
          </div>
        </div>
      </div>
    </div>
